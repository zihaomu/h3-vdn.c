...
static int run_path(h3_gpu *gpu, h3_gpu_tensor *output,
    ...
    setenv("H3_F32_SDPA_SPLIT_SCORES", split_scores ? "1" : "0", 1);
...
int main(int argc, char **argv) {
    ...
    const char *split_first_value = getenv("H3_SDPA_BENCH_SPLIT_FIRST");
    int split_first = split_first_value && *split_first_value &&
                      strcmp(split_first_value, "0");
    ...
    if (ok && wave_first)
        ok = run_path(gpu, output, query, key, value, sequence, heads,
                      iterations, 1, 0, &wave_seconds, wave, elements) &&
             run_path(gpu, output, query, key, value, sequence, heads,
                      iterations, 0, 0, &scalar_seconds, scalar, elements);
    else if (ok)
        ok = run_path(gpu, output, query, key, value, sequence, heads,
                      iterations, 0, 0, &scalar_seconds, scalar, elements) &&
             run_path(gpu, output, query, key, value, sequence, heads,
